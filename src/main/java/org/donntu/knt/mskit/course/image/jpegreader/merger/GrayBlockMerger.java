package org.donntu.knt.mskit.course.image.jpegreader.merger;


import org.donntu.knt.mskit.course.image.jpegreader.MCUBlockHolder;
import org.donntu.knt.mskit.course.image.jpegreader.utils.YUV;
            ...
            for (int yIndex = 0; yIndex < 8; yIndex++) {
                for (int xIndex = 0; xIndex < 8; xIndex++) {
                    if (x + xIndex < width && y + yIndex < height) {
                        fullBlock[(y + yIndex) * width + x + xIndex] = YUV.toGray(blockY[yIndex][xIndex]);
                    }
                }
            }
